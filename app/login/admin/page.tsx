...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from('officers')
        .select('*')
        .eq('designation', 'STATE_ADMIN')
        .order('name');
      if (data) setAdmins(data as Officer[]);
      setLoading(false);
    })();
  }, []);
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selected) return;
    setActor({ role: 'admin', officerId: selected });
    ...
    <div className="mx-auto max-w-md px-4 py-16">
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            State Department Administrator enforcement portal
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="admin">Administrator Profile</Label>
              <select
                id="admin"
                value={selected}
              ...
              >
                <option value="">
                  {loading ? 'Loading...' : 'Select your admin profile'}
                </option>
                {admins.map((adm) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                id="email"
                type="email"
                placeholder="admin@legalmetro.gov.in"
                defaultValue=""
              />
            </div>
            <div className="space-y-2">
          ...
          <p className="mt-4 text-center text-xs text-muted-foreground">
            Demo portal — select any administrator to continue.
          </p>
        </CardContent>
      </Card>
    </div>
